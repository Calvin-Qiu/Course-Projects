...
final class Controller {
    ...
    private Command[] _commands = {
        ...
        new Command(String.format("%s-%s\\(%s\\)$", SQ, SQ, SQ), this::doMove),
        new Command(String.format("%s\\s+%s\\s+%s$", SQ, SQ, SQ), this::doMove),
        new Command("auto\\s+(.+)$", this::doAuto),
        new Command("manual\\s+(.+)$", this::doManual)
    };
    ...
    /** Command "move" for MOVE. */
    private void doMove(Matcher move) {
        Square from, to, spear;
        from = sq(move.group(1));
        to = sq(move.group(2));
        spear = sq(move.group(3));
        Move m = Move.mv(from, to, spear);
        if (board().isLegal(m)
                && board().turn() == board().get(from)) {
            _board.makeMove(m);
            _winner = board().winner();
            if (_winner != null && _winner != board().turn()) {
                if (_winner == WHITE) {
    ...
    /** Command "auto" for END. */
    private void doAuto(Matcher end) {
        String player = end.group(1);
        ...
        if (player.equals("white")) {
            ...
            throw error("player is either black or white");
        }
    }

    /** Command "auto" for END. */
    private void doManual(Matcher end) {
        String player = end.group(1);
        player = player.toLowerCase().trim();
        if (player.equals("white")) {
