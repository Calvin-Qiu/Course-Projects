...
int main( int argc, char** argv ) {
  ...
  while ( (opt = getopt(argc, argv, "d:b:s:q:l:t:m:e:h:H:f:r:c:a:p:")) != -1 ) {  // for each option...
    switch ( opt ) {
    	case 'd':
    		delay = atof(optarg);
    		break;
    	case 'b':
    		...
    		bx = atoi(argv[optind]);
    		by = atoi(argv[optind+1]);
    		bz = atoi(argv[optind+2]);
				beta = {bx, by, bz};
				beta.normalize();
				beta *= b;
				optind += 3;
      ...
      case 's':
          ...
          break;
			case 'q':
          config.pathtracer_ns_pp = atoi(optarg);
          break;
      case 'l':
  ...
  if (write_to_file) {
    ...
    app->boost_camera(beta);

    app->delay_camera(delay);
