...
Ray Camera::generate_ray(double x, double y) const {
  ...
  double X_prime = - (x - 0.5) * deltaX;
  double Y_prime = - (y - 0.5) * deltaY;
  double Z_prime = 1; // distance to sensor
  Vector3D sensor_prime = {X_prime, Y_prime, Z_prime};
  Vector3D sensor = {X_prime, gamma * (Y_prime + beta.y * 5), Z_prime};
//  Vector4D hole = {0, 0, - gamma * beta.z * sensor_prime.norm()};
	double t_prime = - sensor_prime.norm() +  5;
	Vector3D hole = {0, gamma * beta.y * t_prime ,0};
  Vector3D direction = hole - sensor; //gamma * (Z_prime + beta.z * r_prime.norm());
//	Vector3D direction = {-X_prime, -Y_prime, -dir_Z};
	direction.normalize();
  double min_t = - nClip / direction.z;
  double max_t = - fClip / direction.z;
	direction = c2w * direction;
  Ray ray = Ray(pos + c2w * sensor, direction);
  ray.min_t = min_t;
  ray.max_t = max_t;
  return ray;
}

Ray Camera::generate_ray_relativistic(double xs, double ys) const {
	double deltaX = 2 * tan((0.5 * radians(hFov)));
  double deltaY = 2 * tan((0.5 * radians(vFov)));
  double x = - (xs - 0.5) * deltaX;
  double y = - (ys - 0.5) * deltaY;
  double z = 1;
  Vector3D sensor3 = {x, y, z};
  Vector4D sensor4 = {delay, x, y, z}; // 4-vector of sensor
	Vector4D light4 = {sensor3.norm(), x, y, z};
	Vector4D sensor4_lab = cam_to_lab * sensor4;
	Vector4D light4_lab = cam_to_lab * light4;
	Vector3D sensor3_lab = {sensor4_lab[1], sensor4_lab[2], sensor4_lab[3]};
	Vector3D direction = {-light4_lab[1], -light4_lab[2], -light4_lab[3]};
	direction.normalize();
	double min_t = - nClip / direction.z;
  double max_t = - fClip / direction.z;
	Ray ray = Ray(pos + c2w * sensor3_lab, c2w * direction);
	ray.min_t = min_t;
  ray.max_t = max_t;
  return ray;
}

Ray Camera::generate_ray_relativistic_by_wavelength(double xs, double ys, float wavelength) const {
	double deltaX = 2 * tan((0.5 * radians(hFov)));
  double deltaY = 2 * tan((0.5 * radians(vFov)));
  double x = - (xs - 0.5) * deltaX;
  double y = - (ys - 0.5) * deltaY;
  ...
  Vector4D sensor4 = {delay, x, y, z}; // 4-vector of sensor
	...
	double min_t = - nClip / direction.z;
  double max_t = - fClip / direction.z;
	wavelength *= light4[0] / light4_lab[0];
	Ray ray = Ray(pos + c2w * sensor3_lab, c2w * direction);
...
void Camera::set_boost() {
	double b2 = beta.norm2();
	double bx = beta.x;
	double by = beta.y;
	double bz = beta.z;
	double g = gamma;
	...
	if (b2 == 0) {
		cam_to_lab = Matrix4x4::identity();
		lab_to_cam = Matrix4x4::identity();
		return;
	}

	double l2c[] = {    g,            -g*bx,            -g*by,            -g*bz,
								   -g*bx, 1+(g-1)*bx*bx/b2,   (g-1)*bx*by/b2,   (g-1)*bx*bz/b2,
								   -g*by,   (g-1)*bx*by/b2, 1+(g-1)*by*by/b2,   (g-1)*by*bz/b2,
								   -g*bz,   (g-1)*bx*bz/b2,   (g-1)*bz*by/b2, 1+(g-1)*bz*bz/b2 };
	lab_to_cam = Matrix4x4(l2c);


	double c2l[] = {    g,            g*bx,            g*by,            g*bz,
								   g*bx, 1+(g-1)*bx*bx/b2,   (g-1)*bx*by/b2,   (g-1)*bx*bz/b2,
								   g*by,   (g-1)*bx*by/b2, 1+(g-1)*by*by/b2,   (g-1)*by*bz/b2,
								   g*bz,   (g-1)*bx*bz/b2,   (g-1)*bz*by/b2, 1+(g-1)*bz*bz/b2 };
	cam_to_lab = Matrix4x4(c2l);
	return;
}
